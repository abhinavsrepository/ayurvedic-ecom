...
public interface OrderRepository extends JpaRepository<Order, UUID> {
  ...
  @Query(
      "SELECT SUM(o.total) FROM Order o WHERE o.createdAt >= :since AND o.status NOT IN ('CANCELLED', 'REFUNDED')")
  BigDecimal sumRevenueSince(@Param("since") Instant since);

  @Query(
      "SELECT AVG(o.total) FROM Order o WHERE o.createdAt >= :since AND o.status NOT IN ('CANCELLED', 'REFUNDED')")
