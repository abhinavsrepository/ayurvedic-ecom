...
public class ProductController {
    ...
    public ResponseEntity<Page<ProductResponse>> listProducts(
        ...
        Page<ProductResponse> products = productService.listProducts(pageable, search, status, category);
    ...
    @Operation(summary = "Upload product media", description = "Get presigned URL for uploading product images")
    @PreAuthorize("hasAnyRole('ADMIN', 'OPS')")
    public ResponseEntity<MediaUploadResponse> getMediaUploadUrl(
            @PathVariable UUID productId,
            @RequestParam String fileName,
            @RequestParam String contentType) {
        MediaUploadResponse response = productService.getMediaUploadUrl(productId, fileName, contentType);
        return ResponseEntity.ok(response);
    }

    // Inner DTOs
    public record BulkImportResponse(int totalRows, int successCount, int failedCount, String[] errors) {}
